...
int main() {
    unsigned char plaintext[8] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 15};
    unsigned char key[8] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    unsigned char ciphertext[8];

    for(unsigned char a = 0; a++ < 255;) {
      plaintext[0] = a;
      for(unsigned char b = 0; b++ < 255;) {
        plaintext[1] = b;
        for(unsigned char c = 0; c++ < 255;) {
          plaintext[2] = c;
          for(unsigned char d = 0; d++ < 255;) {
            plaintext[3] = d;
            des_encrypt(plaintext, key, ciphertext);
            if (memcmp(plaintext, ciphertext, 8) == 0) {
              printf("found fixed point!\n");
              for(int q = 0; q < 8; q++)
                printf("%02x", plaintext[q]);
              printf("\n");
              for(int q = 0; q < 8; q++)
                printf("%02x", ciphertext[q]);
              printf("\n");
              return 0;
            }
          }
